import * as activityDataFunctions from "./activity.js";
import * as listsDataFunctions from "./lists.js";
import * as trendingDataFunctions from "./trending.js";
import * as userDataFunctions from "./users.js";
import * as statisticsDataFunctions from "./statistics.js";

export const activityData = activityDataFunctions;
export const listData = listsDataFunctions;
export const trendingData = trendingDataFunctions;
export const userData = userDataFunctions;
export const statsData = statisticsDataFunctions;
